...
"@
    ...
    'status' {
        # Single-line condition avoids the PS 5.1 switch-block parse ambiguity
        # that triggers when Select-String appears at the start of a continuation
        # line and the parser mistakes it for a new switch case.
        if ((Test-Path $ProfilePath) -and (Select-String -Path $ProfilePath -Pattern $Marker -Quiet)) {
